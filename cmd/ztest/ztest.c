...
#ifdef __APPLE__
		uint32_t execnamesize = MAXPATHLEN;
		char *execname = umem_alloc(MAXPATHLEN, UMEM_NOFAIL);
		VERIFY(0 == getexecname(execname, &execnamesize));
		VERIFY(NULL != realpath(execname, cmd));
		umem_free(execname, MAXPATHLEN);
#else
...
ztest_run_zdb(char *pool)
{
	...
	char *execname = umem_alloc(MAXPATHLEN, UMEM_NOFAIL);
	VERIFY(0 == getexecname(execname, &execnamesize));
	VERIFY(realpath(execname, bin) != NULL);
	umem_free(execname, MAXPATHLEN);
#else
...
#ifdef __APPLE__
		uint32_t cmdsize = MAXPATHLEN;
		VERIFY(0 == getexecname(cmdbuf, &cmdsize));
#else
...
#ifdef __APPLE__
	uint32_t cmdsize = MAXPATHLEN;
	VERIFY(0 == getexecname(cmd, &cmdsize));
#else
