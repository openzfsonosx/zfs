...
get_diskutil_cs_info(char *slice, DU_Info *info)
{
	int status = 0;
	int out_length = 0;
	char *cc[] = {"/usr/sbin/diskutil", "cs", "info", "-plist", slice, NULL};
	char *output = NULL;
	CFPropertyListRef plist = NULL;
	CFStringRef slice_str = CFStringCreateWithCString(NULL, slice, kCFStringEncodingUTF8);
			...
			if (output) {
				...
				if (bytes) {
					plist = CFPropertyListCreateWithData(NULL,
					    bytes, kCFPropertyListImmutable, NULL, &err);

					if (plist)
						CFDictionaryAddValue(diskutil_cs_info_cache, slice_str, plist);
		...
		if (WIFEXITED(status) && (WEXITSTATUS(status) == 0)) {
			...
			if (output) {
				...
				if (bytes) {
					...
					if (plist)
						CFDictionaryAddValue(diskutil_info_cache, slice_str, plist);
...
get_cs_LV_status(DU_Info *info)
{
	...
	if (info &&
	    CFDictionaryGetValueIfPresent((CFDictionaryRef)info, CFSTR("CoreStorageLogicalVolumeStatus"),
	    (const void **)&existing)) {
		return existing;
	}
...
is_whole_disk(DU_Info info)
{
	...
	if (info &&
	    CFDictionaryGetValueIfPresent((CFDictionaryRef)info, CFSTR("WholeDisk"),
		(const void **)&is_whole)) {
		ret = is_whole;
	}
...
{
	return (info &&
	CFDictionaryContainsKey((CFDictionaryRef)info, CFSTR("FilesystemType")));
}
...
get_filesystem_type(DU_Info info)
{
	...
	if (info &&
	    CFDictionaryGetValueIfPresent((CFDictionaryRef)info, CFSTR("FilesystemType"),
	    (const void **)&existing)) {
		return existing;
	}
