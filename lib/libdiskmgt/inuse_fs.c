...
inuse_fs(char *slice, nvlist_t *attrs, int *errp)
{
	...
	if (diskutil_info_valid(info) && has_filesystem_type(info)) {
		...
		Boolean success = CFStringGetCString(filesystem_type, filesystem_type_str, 128, kCFStringEncodingUTF8);

		if (filesystem_type &&
		    (CFStringCompare(filesystem_type, CFSTR("zfs"), kCFCompareCaseInsensitive) != kCFCompareEqualTo)) {
