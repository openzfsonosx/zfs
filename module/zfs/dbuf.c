...
dbuf_include_in_metadata_cache(dmu_buf_impl_t *db)
{
	...
	if (DMU_OT_IS_METADATA_CACHED(type)) {
		 ...
		 */
		if (zfs_refcount_count(&dbuf_caches[DB_DBUF_METADATA_CACHE].size) >
...
dbuf_evict_one(void)
{
	...
	if (db != NULL) {
		...
		(void) zfs_refcount_remove_many(&dbuf_caches[DB_DBUF_CACHE].size,
		    db->db.db_size, db);
...
__dbuf_hold_impl(struct dbuf_hold_impl_data *dh)
{
	...
	*(dh->dh_dbp) = NULL;
	...
	if (multilist_link_active(&dh->dh_db->db_cache_link)) {
		...
		multilist_remove(dbuf_caches[dh->dh_db->db_caching_status].cache, dh->dh_db);
		(void) zfs_refcount_remove_many(
 ...
 */
int
dbuf_hold_impl(dnode_t *dn, uint8_t level, uint64_t blkid, boolean_t fail_sparse,
    boolean_t fail_uncached, void *tag, dmu_buf_impl_t **dbp)
{
