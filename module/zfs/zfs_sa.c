...
sa_attr_reg_t zfs_attr_table[ZPL_END+1] = {
	...
	{"ZPL_DXATTR", 0, SA_UINT8_ARRAY, 0},
        ...
        {"ZPL_ALT_PARENT", sizeof (uint64_t), SA_UINT64_ARRAY, 0},
        {"ZPL_FINDERINFO", 32, SA_UINT8_ARRAY, 0},
#endif
	{NULL, 0, 0, 0}
};
...
zfs_sa_upgrade(sa_handle_t *hdl, dmu_tx_t *tx)
{
         ...
         */
        if (zp->z_acl_cached == NULL || ZP_GET_VTYPE(zp) == VLNK)
                return;
        ...
        SA_ADD_BULK_ATTR(sa_attrs, count, SA_ZPL_LINKS(zfsvfs), NULL,
            &zp->z_links, 8);
        if (ZP_GET_VTYPE(zp) == VBLK || ZP_GET_VTYPE(zp) == VCHR)
