...
{
#ifdef DEBUG
	VERIFY(abd->abd_magic == ABD_DEBUG_MAGIC);
#endif
...
abd_get_from_buf(void *buf, size_t size)
{
	abd_t *abd = abd_alloc_struct(0);
#ifdef DEBUG
	VERIFY(((abd_t *)buf)->abd_magic != ABD_DEBUG_MAGIC);
#endif
...
#ifdef DEBUG
	if (((abd_t *)buf)->abd_magic == ABD_DEBUG_MAGIC)
		panic("%s: buf ptr contains ABD magic", __func__);
#endif
...
{
#ifdef DEBUG
	if (((abd_t *)dbuf)->abd_magic == ABD_DEBUG_MAGIC)
		panic("%s: buf ptr contains ABD magic", __func__);
#endif
